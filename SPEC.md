...
13. **"X & Y" + range language → one range item.** When multiple dates like "X & Y" appear together WITH range language ("through the Nth", "until the Nth", "open X–Y", "available X through Y"), treat them as ONE date-range item ending on the Nth (`all_day: true`, `is_date_range: true`, `range_end_date` = the Nth), with the featured dates noted in `notes`. Absent explicit range language, distinct dates remain distinct items (rule 6/12). Example (Nando's × Utopia Bagels): "July 4 & 11, available through the 11th" → one all-day range July 4→11, not two single-day items.
